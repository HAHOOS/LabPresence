...
namespace LabPresence.Managers
{
    internal static class MenuManager
    {
        ...
        public static void PopulateConfig()
        {
            ...
            ConfigPage.CreateBool("Enabled", FromRGB(170, 51, 255), Core.Config.Enabled, (val) => { Core.Config.Enabled = val; Core.Category.SaveToFile(false); if (!Core.Config.Enabled) Core.Client?.ClearPresence(); }).SetTooltip("If false, the mod will not update the Discord Rich Presence");
            ConfigPage.CreateInt("Refresh Time", FromRGB(255, 56, 255), Core.Config.RefreshTime, 1, 5, 60 * 60, (val) => { Core.Config.RefreshTime = val; Core.Category.SaveToFile(false); }).SetTooltip("The interval in which the rich presence should be updated with up-to-date placeholder values");

            ConfigPage.CreateEnum("RPC Log Level", FromRGB(255, 172, 28), Core.Config.RPCLogLevel, (val) => { Core.Config.RPCLogLevel = (LogLevel)val; Core.Category.SaveToFile(false); Core.Client?.Logger?.Level = (LogLevel)val; }).SetTooltip("The logs of RPC that will be displayed, available: None, Trace, Info, Warning, Error");
            ConfigPage.CreateEnum("Time Mode", FromRGB(191, 255, 0), Core.Config.TimeMode, (val) => { Core.Config.TimeMode = (TimeMode)val; Core.Category.SaveToFile(false); Core.ConfigureTimestamp(true); }).SetTooltip("What the Rich Presence will display as time, available options:\n-> Level (since the current level was loaded)\n-> CurrentTime (the current time, example: 15:53:50)\n-> GameSession (since the game was launched)");
